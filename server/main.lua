...
CreateThread(function()
    exports["esx_society"]:registerSociety('cardealer', _U('car_dealer'), 'society_cardealer', 'society_cardealer', 'society_cardealer', { type = 'private' })
end)
...
AddEventHandler('esx_vehicleshop:setVehicleOwnedPlayerId', function(playerId, vehicleProps, model, label)
    ...
    if xPlayer.job.name ~= 'cardealer' or not xTarget then return end
...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function(source, cb, model)
    local xPlayer = ESX.GetPlayerFromId(source)
    local modelPrice = getVehicleFromModel(model).price

    if not ESX.GetVehicleData(model) then return ESX.Trace(("Vehicle (%s) does not exist in the framework's vehicles.json file!"):format(model), "error", true) end
    ...
    if modelPrice and xPlayer.getMoney() >= modelPrice then
        ...
        local vehicle = ESX.CreateVehicle({
            ...
            model = model
        }, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading)
