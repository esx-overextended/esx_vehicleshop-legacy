...
local function PlayerManagement()
    if not Config.EnablePlayerManagement then return end

    Wait(10) -- wait for the player data to update
...
do TriggerServerEvent("esx_vehicleshop:getVehiclesAndCategories") end
...
RegisterNetEvent('esx_vehicleshop:updateVehiclesAndCategories', function(vehicles, categories, vehiclesByModel)
    ...
    for _, vehicle in ipairs(Vehicles) do
        if IsModelInCdimage(joaat(vehicle.model)) then
            ...
            if not vehiclesByCategory[category] then vehiclesByCategory[category] = {} end
